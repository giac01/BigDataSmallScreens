...
#df0[df0$SchoolID==99,]=NA #Commented out - use data for all children
...
print(plot(IRT_Cat$mirt_model, type="trace",theta_lim=c(-5,5)))
print(plot(IRT_Cat$mirt_model, type="info",theta_lim=c(-5,5)))
print(plot(IRT_Cat$mirt_model, type="rxx",theta_lim=c(-5,5)))
...
#Vocab test 
IRT_Vocab =IRTcheck(vars="Vocab_item",mirtoutput =TRUE, mirt_modeltype="Rasch", nCores=NumberOfCores, mirt_Method=mirt_Method, mirt_Optimizer=mirt_Optimizer, guess_param = 1/4)

# apply(df0[,findMCS("Vocab_item")]))
fa(na.omit(df0[,findMCS("Vocab_item")]))
cronbach.alpha((na.omit(df0[,findMCS("Vocab_item")])))
...
PlotTasks = rbind.data.frame(IRT_Cat2$mirt_plotdata,IRT_ANS$mirt_plotdata,IRT_PD2$mirt_plotdata,IRT_RJ2$mirt_plotdata,IRT_RJ$mirt_plotdata,IRT_Vocab$mirt_plotdata)
...
PlotTasks$task=recode(PlotTasks$task, cattell="Matrix Reasoning",rj="Rhyme Judgement (6 items)", rjcomplete="Rhyme Judgement (22 items)",pd="Phonological Discrimination",ans="Non-Symbolic Num. Discrim.", vocab="Picture Vocabulary")

labels=c("Matrix Reasoning","Rhyme Judgement (6 items)","Rhyme Judgement (22 items)","Phonological Discrimination","Non-Symbolic Num. Discrim.", "Picture Vocabulary")
...
IRT_Plot=ggplot(PlotTasks,aes(x=UniRange,y=mirt_rel,group=task, color=task)) + 
  ...
  jtools::theme_apa() + 
  geom_hline(yintercept = .6, linetype="dotted") + labs(x="True Score (Percentile)",y="Conditional Reliability Alpha", title="") +
...
save(IRT_Plot, file=file.path("Plots","IRT_Plot_CompleteSample"))
#pdf(file="Plots/IRT.pdf", width=6,height=4)
IRT_Plot
dev.off()
# 
...
IRT_Vocab_Score = IRT_Vocab$mirt_scores
IRT_Vocab_ScoreNorm = Normalise(IRT_Vocab_Score)
